...
static unsigned char png_sig[8] = {0x89, 0x50, 0x4e, 0x47,
				   0x0d, 0x0a, 0x1a, 0x0a};
...
unsigned char inout(FILE *fp_in, FILE *fp_out) {
	int val;
...
unsigned char inonly(FILE *fp_in) {
	int val;
...
unsigned char outonly(unsigned char c, FILE *fp_out) {
	int val;
	...
	for (i = 0; i < 8; i++) {
		...
		if ((val = fgetc(fp_in)) == EOF) {
			...
			if (val != (int)png_sig[i]) {
				fprintf(
				    stderr,
				    "%s: PNG signature error in %s (byte %d)\n",
				    argv[0], argv[1], i);
				exit(1);
			}
		}
